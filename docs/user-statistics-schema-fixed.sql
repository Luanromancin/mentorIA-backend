-- Script corrigido para resolver problemas de permissão
-- Execute este script no SQL Editor do Supabase

-- Habilitar a extensão uuid-ossp se ainda não estiver habilitada
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Remover funções e gatilho se existirem para permitir a reexecução do script
DROP TRIGGER IF EXISTS update_user_statistics_updated_at ON user_statistics;
DROP FUNCTION IF EXISTS update_updated_at_column;
DROP FUNCTION IF EXISTS upsert_user_statistics;
DROP FUNCTION IF EXISTS get_user_statistics;

-- Remover políticas antigas se existirem
DROP POLICY IF EXISTS "Usuários podem ver suas próprias estatísticas" ON user_statistics;
DROP POLICY IF EXISTS "Usuários podem inserir suas próprias estatísticas" ON user_statistics;
DROP POLICY IF EXISTS "Usuários podem atualizar suas próprias estatísticas" ON user_statistics;
DROP POLICY IF EXISTS "Permitir acesso para funções SECURITY DEFINER" ON user_statistics;
...
-- Criar tabela user_statistics (se não existir)
...
-- Política única que permite acesso total para funções SECURITY DEFINER
-- Esta política é necessária para que as funções SECURITY DEFINER funcionem
