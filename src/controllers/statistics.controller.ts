...
export class StatisticsController {
    ...
    async getUserStatistics(req: Request, res: Response): Promise<Response> {
        try {
            ...
            const statistics = await this.statisticsService.getUserStatistics(profileId);
     ...
     */
    async getCompetencyStatistics(req: Request, res: Response): Promise<Response> {
        try {
            const profileId = (req as any).user?.id;
            if (!profileId) {
                return res.status(401).json({
            ...
            const competencyStats = await this.statisticsService.getUserCompetencyStatistics(profileId);
    ...
    async getCompetencyProgress(req: Request, res: Response): Promise<Response> {
        try {
            ...
            const competencyProgress = await this.statisticsService.getCompetencyProgress(profileId);
    ...
    async saveUserAnswer(req: Request, res: Response): Promise<Response> {
        try {
            ...
            const { questionId, selectedAlternativeId, isCorrect, timeSpentSeconds } = req.body;
