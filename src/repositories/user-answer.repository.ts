import { UserAnswer, CreateUserAnswerDto, UserAnswerWithDetails } from '../entities/user-answer.entity';
...
export class UserAnswerRepository {
    ...
    async getStatisticsByTopic(profileId: string): Promise<{
        ...
        try {
            ...
            for (const row of data || []) {
                ...
                const subtopic = (row.questions as any)?.subtopic_name || 'Sem subtópico';
            ...
            return Array.from(topicStats.values()).map(topic => ({
                ...
                subtopics: Array.from(topic.subtopics.values()).map(subtopic => ({
                    ...
                    accuracy: subtopic.totalQuestions > 0 ? (subtopic.correctAnswers / subtopic.totalQuestions) * 100 : 0
                }))
            }));
        } catch (error) {
            console.error('Erro ao buscar estatísticas por tópico:', error);
     ...
     */
    async getRecentActivity(profileId: string, days: number = 7): Promise<{
        date: string;
    ...
    async getUserStatistics(profileId: string): Promise<{
        ...
        try {
            ...
            const correctAnswers = basicData?.filter(row => row.is_correct).length || 0;
            const accuracy = totalQuestions > 0 ? (correctAnswers / totalQuestions) * 100 : 0;

            // Tempo médio por questão
            const timeData = basicData?.filter(row => row.time_spent_seconds) || [];
            const averageTimePerQuestion = timeData.length > 0
                ? timeData.reduce((sum, row) => sum + (row.time_spent_seconds || 0), 0) / timeData.length
                : 0;
            ...
            if (streakData && streakData.length > 0) {
                const uniqueDates = new Set(
                    streakData.map(row => new Date(row.answered_at).toISOString().split('T')[0])
                );
            ...
            if (sessionsData && sessionsData.length > 0) {
                ...
                for (let i = 1; i < sessionsData.length; i++) {
                    const currentAnswerTime = new Date(sessionsData[i].answered_at).getTime();
