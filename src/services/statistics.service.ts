...
export class StatisticsService {
    ...
    async getUserStatistics(profileId: string): Promise<UserStatistics> {
        try {
            ...
            const basicStats = await this.userAnswerRepository.getUserStatistics(profileId);

            // Buscar estatísticas por tópico
            const topicStats = await this.userAnswerRepository.getStatisticsByTopic(profileId);

            // Buscar atividade recente
            const recentActivity = await this.userAnswerRepository.getRecentActivity(profileId, 7);
            ...
            const totalStudyTime = Math.floor(basicStats.averageTimePerQuestion * basicStats.totalQuestions / 60); // em minutos
            ...
            console.log(`✅ Estatísticas carregadas: ${statistics.totalQuestions} questões, ${statistics.accuracy}% de precisão`);
            return statistics;
        } catch (error) {
    ...
    async saveUserAnswer(data: {
        ...
        try {
            console.log(`💾 Salvando resposta: questão ${data.questionId}, correto: ${data.isCorrect}`);
    ...
    async getUserCompetencyStatistics(profileId: string): Promise<{
        ...
        try {
            ...
            const masteredCompetencies = userCompetencies.filter(comp => comp.level === 3).length;
            const inProgressCompetencies = userCompetencies.filter(comp => comp.level === 1 || comp.level === 2).length;
            const beginnerCompetencies = userCompetencies.filter(comp => comp.level === 0).length;

            const averageLevel = totalCompetencies > 0
                ? userCompetencies.reduce((sum, comp) => sum + comp.level, 0) / totalCompetencies
                : 0;
    ...
    async getCompetencyProgress(profileId: string): Promise<{
        ...
        try {
            const userCompetencies = await this.userCompetencyRepository.findByProfileId(profileId);
            const userAnswers = await this.userAnswerRepository.findByProfileId(profileId);

            // Agrupar respostas por competência
            const competencyStats = new Map<string, {
                totalQuestions: number;
                correctAnswers: number;
            }>();
            ...
            return userCompetencies.map(competency => {
                const stats = competencyStats.get(competency.competency?.name || '') || {
                    totalQuestions: 0,
                    correctAnswers: 0
                };
